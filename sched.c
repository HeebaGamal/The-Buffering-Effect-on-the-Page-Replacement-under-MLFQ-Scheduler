...
struct Env* fos_scheduler_MLFQ()
{
	...
	if(curenv!=NULL)
	{
		int index_priority = curenv->priority;
		if(index_priority==num_of_ready_queues)
			LIST_INSERT_HEAD(&(env_ready_queues[index_priority]), curenv);
		else
			LIST_INSERT_HEAD(&(env_ready_queues[index_priority+1]), curenv);
	}
	//[1] If the current environment (curenv) exists, place it in the suitable queue

		struct Env *process_env ;
		//[2] Search for the next env in the queues according to their priorities (first is highest)

		for(int i = 0; i<num_of_ready_queues; i++) {
			process_env = LIST_LAST(&(env_ready_queues[i])) ;
			int m  = -1;
			if(process_env)
			{
				//cprint("process \n");
				kclock_set_quantum(quantums[i]);
				LIST_REMOVE(&(env_ready_queues[i]), process_env);
				return process_env;
				//m = 1;
			}
			/*
			 if(m==1)
			 	 return process_env;
			 */
			/*LIST_FOREACH(process_env, &(env_ready_queues[i]))
			{
				if(process_env) {
					kclock_set_quantum(quantums[i]);
					LIST_REMOVE(&(env_ready_queues[i]), process_env);
